...
client.on("guildCreate", guild => {
  ...
  const tesekkurler = new Discord.MessageEmbed()
    .setTitle(`Replace Bilgilendirme`)
    .setTimestamp()
    .setColor("GREEN")
    .setDescription(
      `Beni Sunucuna Eklediğin İçin Teşekkür Ederim \n Sana En İyi Şekilde Hizmet Edeceğim.\n Eğer Bir Sorunla Karşılaşırsan Destek Sunucuma Gel https://discord.gg/qa4xqCNNME \n Komutlarımız için **r_yardım** komutunu kullanınız.`
    );
...
if (!rol) return;
  ...
  let embed = new Discord.MessageEmbed()
    .setDescription(`:mega: Sunucuya yeni katılan **${member.user.username}** kullanıcısına **${rol}** rolü verildi <a:evet:784518934711762976>`)
...
let kanal = await db.fetch(`antiraidK_${member.guild.id}`)== "bot-koruma aç"
  ...
  if (member.user.bot === true) {
    ...
    let are = new Discord.MessageEmbed()
      ...
      .setDescription(`<a:r_evet:785437821926113290> **${member.user.tag}** (${member.id}) adlı bota bir yetkili verdi eğer kaldırmak istiyorsanız **${prefix}bot-izni kaldır botun_id**.`);
    cod.send(are);//CodAre✨
     } else {
       let izinverilmemişbot = new Discord.MessageEmbed()
      .setColor("RANDOM")
      .setThumbnail(member.user.avatarURL({dynamic: true}))
      .setDescription("<a:r_evet:785437821926113290> **" + member.user.tag +"**" + " (" + member.id+ ") " + "adlı bot sunucuya eklendi ve banladım eğer izin vermek istiyorsanız **" + prefix + "bot-izni ver botun_id**")
         ...
         client.on("roleDelete", async(role , channel , message , guild) => {
          ...
          role.guild.createRole({name: role.name, color: role.color,  permissions: role.permissions}) 
                role.guild.owner.send(
                  ...
                  .setDescription(`<a:evet:784518934711762976> **${role.name}** Adlı Rol Silindi Ve Ben Rolü Tekrar Oluşturdum`))
