...
exports.run = (client, message, args) => {
  if (args[0] == "aç") {
    if (db.has(`antiraidK_${message.guild.id}`) === true) {
      ...
      .setColor("#501c67")
      .setDescription("<a:XTik:798277028050501642> Bot koruma sistemi zaten açılmış."));
    }
    db.set(`antiraidK_${message.guild.id}`, "bot-koruma aç");
    const açıldı = new Discord.MessageEmbed()
    .setColor("#501c67")
    .setDescription("<a:tik1:798276995011051530> Bot koruma sistemi başarıyla açıldı")
  ...
  if (args[0] == "kapat") {
    if (db.has(`antiraidK_${message.guild.id}`) === false) {
      ...
      .setDescription("<a:XTik:798277028050501642> Bot koruma sistemi açılmamış. Açmak için **bot-koruma aç**")
  ...
  if (!args[0])
    return message.channel.send(
      new Discord.MessageEmbed()
      .setColor("#501c67")
      .setDescription("<a:XTik:798277028050501642> Lütfen geçerli işlem girin. Örnek: **bot-koruma aç/kapat**")
    );
};
exports.conf = {
  enabled: true,
