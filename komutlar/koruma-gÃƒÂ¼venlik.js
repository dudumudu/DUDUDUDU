...
let prefix = ayarlar.prefix
   ...
   if(!kanal) return message.channel.send(
     ...
     .setColor("#501c67")
     .setDescription('<a:tik1:798276995011051530> Güvenlik mesajlarının gideceği kanalı etiketlemedin.'))
   else {
    db.set(`güvenlik.${message.guild.id}`, kanal.id)
    return message.channel.send(
      new Discord.MessageEmbed()
      .setColor("#501c67")
      .setDescription('<a:tik1:798276995011051530> Güvenlik kanalı <#'+kanal+'> olarak ayarlandı.'))
   }
   if(args[0] === 'sıfırla') {
    db.delete(`güvenlik.${message.guild.id}`)
     const sıfırladın = new Discord.MessageEmbed()
     .setColor("#501c67")
     .setDescription('<a:tik1:798276995011051530> Güvenlik kanalı başarıyla sıfırlandı.')
